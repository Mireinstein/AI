...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # A state will be described by the current position and a tuple of the corners(tuples also, as, x,y)
        # that have been visited. Note than pacman may start in a corner and we may want to capture that
        if self.startingPosition in self.corners:
            self.start_state = (self.startingPosition, (self.startingPosition,))
        else:
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # A state is a goal state if the number of visited corners is four
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # We only add a successor state to the search state if the action did not move through the wall.
            if not hitsWall:
                # if we end up at a corner that we have not visited in the state before,
                # we add it to the successor state's visited corners
                if (nextx, nexty) in self.corners and (nextx, nexty) not in state[1]:
                    visited_corners = state[1] + ((nextx, nexty),)
                else:
                    visited_corners = state[1]
                successor = ((nextx, nexty), visited_corners)
